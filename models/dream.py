...
class DrafterModel(nn.Module):

    def __init__(self, config):
        super().__init__()
        self.config = config 
        base_config = AutoConfig.from_pretrained(config.model.base_model, trust_remote_code=True)


        param_copies = 1 if config.model.shared_params else config.model.speculation_heads

        if config.training.layerwise_feats : 
            self.features_linear_layer = torch.nn.Linear(4*base_config.hidden_size, base_config.hidden_size)

        self.down_project = nn.ModuleList(
            [nn.Linear(2*base_config.hidden_size, base_config.hidden_size) for _ in range(param_copies)]
        )

        if (config.model.disable_attn):
            self.mlp = nn.ModuleList(
                [nn.Linear(base_config.hidden_size, base_config.hidden_size) for _ in range(param_copies)]
            )
        else : 
            self.layers = nn.ModuleList(
                [nn.ModuleList([DreamDecoderLayer(base_config, layer_idx) for layer_idx in range(config.model.n_layers)]) for _ in range(param_copies)]
            )

        self.norm_embed = nn.ModuleList(
            [DreamRMSNorm(base_config.hidden_size, eps=base_config.rms_norm_eps) for _ in range(param_copies)]
        )
        self.norm_hidden = nn.ModuleList(
            [DreamRMSNorm(base_config.hidden_size, eps=base_config.rms_norm_eps) for _ in range(param_copies)]
        )
        self.norm_postattn = nn.ModuleList(
            [DreamRMSNorm(base_config.hidden_size, eps=base_config.rms_norm_eps) for _ in range(param_copies)]
        )
        
        self.rotary_emb = DreamRotaryEmbedding(config=base_config)


    def tie_weights(self):
        pass

    def forward(self, input_embeds, prior_hidden_states, attention_mask, position_ids, param_idx):
        param_idx = 0 if self.config.model.shared_params else param_idx

        input_embeds_normalized = self.norm_embed[param_idx](input_embeds)
        prior_hidden_states_normalized = self.norm_hidden[param_idx](prior_hidden_states)

        hidden_states = torch.cat([input_embeds_normalized, prior_hidden_states_normalized],dim=-1)
        ...
        # decoder layers
        hidden_states = self.down_project[param_idx](hidden_states)

        if (self.config.model.disable_attn):
            hidden_states = self.mlp[param_idx](hidden_states.clamp(min=0))
        else : 
            for decoder_layer in self.layers[param_idx]:
        ...
        hidden_states = self.norm_postattn[param_idx](hidden_states)
