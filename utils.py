...
def truncate_probs(p_x0,top_p):
  sorted_probs, sorted_indices = torch.sort(p_x0, descending=True, dim=-1)
  cumulative_probs = torch.cumsum(sorted_probs, dim=-1)
  top_p_mask = cumulative_probs <= top_p
  top_p_mask[..., 0] = True
  nucleus_probs = torch.where(top_p_mask, sorted_probs, torch.tensor(0.0, device=p_x0.device))
  sum_nucleus_probs = nucleus_probs.sum(dim=-1, keepdim=True)
  nucleus_probs = nucleus_probs / sum_nucleus_probs
  p_x0 = torch.zeros_like(p_x0).scatter_(-1, sorted_indices, nucleus_probs)
  return p_x0


def temperate_probs(p_x0,temp):
  new_logits = torch.log(p_x0)/(temp+1e-5)
  new_p_x0 = F.softmax(new_logits,dim=-1)
  return new_p_x0
